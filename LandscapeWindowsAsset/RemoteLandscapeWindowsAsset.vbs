...
arrFileLines = ReadInFile(".\DUB-hosts.txt")
For lineCount = LBound(arrFileLines) To UBound(arrFileLines)
	
	On Error Resume Next
		Set objWMIService = GetObject("winmgmts:\\" & arrFileLines(lineCount) & "\root\CIMV2")
		Dim serialNumber, model, manufacturer, dnsHostname, username
		
		If Err.Number <> 0 Then
			WScript.Echo "Error Communicating to : " & arrFileLines(lineCount)
		Else		
			...
			Else
				...
				For Each comp In objCompSystem
					...
					manufacturer = rtrim(comp.Manufacturer)
					dnsHostname = RTrim(comp.DNSHostName)
					username = RTrim(comp.UserName)
				Next
			End If
			...
			WScript.Echo dnsHostname & vbTab & serialNumber & vbTab & manufacturer & vbTab & model & vbTab & username
...
Private Function ReadInFile(ByVal filename)

Dim arrLines
Dim objFSO, objTxtFile
Set objFSO = CreateObject("Scripting.FileSystemObject")
If objFSO.FileExists( filename ) Then
	Set objTxtFile = objFSO.OpenTextFile (filename, FOR_READING)
Else
	WScript.echo "ERROR FILE NOT FOUND"
	WScript.Quit
End If

arrLines = Split(objTxtFile.ReadAll, VbCrLf)
ReadInFile = arrLines
...
Private Sub getSerial
	On Error Resume Next
		...
		If Err.Number <> 0 Then
			Wscript.Echo " Serial Number: ERROR"			
		End If
...
Private Sub getModel	
	On Error Resume Next
		...
		If Err.Number <> 0 Then
			WScript.Echo " Model: ERROR"
			WScript.Echo " Manufacturer: ERROR"
		End If
		
		Dim comp
		For Each comp In objCompSystem
			Wscript.Echo " Model: " & rtrim(comp.model)
			WScript.Echo " Manufacturer: " & rtrim(comp.Manufacturer)
		Next
	Err.Clear
End Sub
