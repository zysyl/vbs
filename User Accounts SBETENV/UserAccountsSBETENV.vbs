...
Const SQL_STATEMENT = "SELECT objectGuid, distinguishedName, samAccountName, lastLogonTimestamp, userAccountControl FROM 'LDAP://DC=Sbetenv,DC=ADS' WHERE objectCategory='user' AND objectClass='user' ORDER BY samAccountName"					  
	...
	Call WriteToLog (glb_Logfilename, BytesToHexString( glb_objADRecordset.Fields("objectGUID") ) & vbTab & _
 										   glb_objADRecordset.Fields("distinguishedName") & vbTab & _
 											glb_objADRecordset.Fields("samAccountName") & vbTab & _
 											 accountEnabled & vbTab & "Generic Account" & vbTab & _
 											  lastLogon )				
...
' AUTHOR NOTE: LEAVE the following in for potential extention in future
' WScript.Echo Date & ": Total User Accounts: " & glb_totalAccounts & VbCrLf
' WScript.Echo "Of this..."
' WScript.Echo " " & glb_totalEnabled & ", accounts currently enabled"
' WScript.Echo "  " & glb_totalEnabledAndGeneric & ", enabled accounts have been detected as being generic"
' WScript.Echo "  " & glb_NotGenericNeverLoggedIn & ", enabled, not generic and never logged in" 
' WScript.Echo "  " & glb_totalValid & ", enabled not generic and have logged in" & VbCrLf 
' WScript.Echo " " & glb_totalDisabled & ", accounts currently disabled"
' 
' WScript.Echo "From this run..." & VbCrLf
' WScript.Echo " " & glb_totalToDisable & " would be disabled"
